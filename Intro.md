...
<h1><p style="text-align:center">프로토타입</p></h1>

# 1. 소개
UI와 데이터 구조 및 디자인 패턴에 중점을 맞춘 로그라이크 류 2D 게임을 만든다. 게임 엔진은 유니티이며 플랫폼은 안드로이드로 결정한다. 

UI는 NGUI 또는 Unity에서 무료로 제공하는 UI 시스템을 사용한다.
...
- 데이터 구조는 다음과 같아야 한다 (수기 작성이 아닌 데이터 편집기가 자동으로 작성하게 된다.)
...
- 맵 데이터 작성기는 C# Winform 또는 QT 5로 작성되며 JSON으로 내보낼 수 있다 (개발 기간 3일 예상)
    ...
    + C#으로 작성한다면 유니티에서 데이터 로더를 구성할 때 더 편리할 것이다.

- 데이터 파일의 이름 명명 규칙은 ```state[스테이지_번호]_monsters.json```으로 결정한다.

## 3.9. 오브젝트
모든 오브젝트는 초기에 맵이 묘화된 후 그 위에 그려지게 된다.
...
- 보물 상자는 지정된 램덤 스팟 (배열 형태)에 초기에 설정된다.
    + 골드 또는 스킬을 얻을 수 있다.
...
2D 좌표계이며 Time.deltaTime을 이용하여 매 프레임마다 일정한 이동을 하게 된다. Grid-Free 또는 픽셀 기반 이동을 택한다.

### 3.12.1. Easing / 타겟에 대한 회전 / 타겟까지 이동

Easing은 iTween 같은 무료 에셋을 사용하고 나머지는 기본 내장 수학 함수로 처리한다.

## 3.13. 충돌

충돌은 유니티 내장 충돌 콜백 이벤트 함수를 사용해야 하며, 기본적인 물리는 무거우므로 계산하지 않아야 한다.

# 4. UI
경험에 의하면 UI는 <a href="https://assetstore.unity.com/packages/tools/gui/ngui-next-gen-ui-2413?locale=ko-KR">NGUI 에셋 (유료)</a>을 이용하여 구성하면 굉장히 편리하다. 그러나 유니티 4.6 이후로 추가된 무료인 UGUI를 대신 사용할 수도 있다. UGUI 사용 시 확장 에셋이 필수이다.
...
# 5. 문서 작성 요령
- 이를 바탕으로 문서를 작성해야 하며 목차가 있어야 한다.
- 이미지 및 표 포함하여 10장 이내로 간단히 작성해야 한다. 
- 복잡한 설명은 별도 첨부하여 원 문서에서 제거하도록 한다.